...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1 text-gray-800 fw-bold">Cập nhật sản phẩm</h1>
        <p class="text-muted small mb-0">Chỉnh sửa thông tin chi tiết sản phẩm.</p>
    </div>
        ...
        <div class="col-lg-8">
            <div class="tech-card mb-4">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" id="name" class="form-control" placeholder="Nhập tên sản phẩm..." required value="{{ old('name', $product->name) }}">
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-bold small text-uppercase text-muted">Mô tả chi tiết</label>
                        <textarea name="description" class="form-control tinymce-editor" rows="8" placeholder="Nhập mô tả sản phẩm...">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="tech-card mb-4">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" id="thumbnail-fake-input" class="form-control" readonly style="cursor: pointer; background-color: #fff;" 
                                value="{{ $product->thumbnail ? basename($product->thumbnail) : 'Chưa có tệp nào được chọn' }}" 
                        ...
                        <div id="thumbnail-preview" class="mt-2">
                            @if($product->thumbnail)
                                <div class="position-relative" style="display: inline-block;" id="existing-thumbnail-container">
                                    <img src="{{ Storage::url($product->thumbnail) }}" alt="Thumbnail" class="rounded shadow-sm border border-secondary border-1 mt-2" style="width: 120px; height: 120px; object-fit: cover;">
                                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger shadow-sm mt-2" style="cursor: pointer;" onclick="document.getElementById('existing-thumbnail-container').style.display='none'; document.getElementById('delete_thumbnail_input').value='1'; document.getElementById('thumbnail-fake-input').value='Chưa có tệp nào được chọn';">
                    ...
                    <hr>
                    @php
                        $videoUrls = isset($product->images['video_urls']) ? $product->images['video_urls'] : [];
                        if (isset($product->images['video_url']) && !empty($product->images['video_url'])) { 
                            $videoUrls[] = $product->images['video_url'];
                        }
                        $videoUrlsStr = implode("\n", $videoUrls);
                        $gallery = isset($product->images['gallery']) ? $product->images['gallery'] : (is_array($product->images) && !isset($product->images['video_url']) && !isset($product->images['video_urls']) ? $product->images : []);
                    @endphp
                    <div class="mb-3 mt-3">
                        <label class="form-label fw-bold small text-uppercase text-muted">Link Video Youtube sản phẩm</label>
                        <textarea name="video_urls" id="video-urls-input" class="form-control" rows="3" placeholder="https://www.youtube.com/watch?v=...&#10;Có thể nhập nhiều link, mỗi link 1 dòng">{{ old('video_urls', $videoUrlsStr) }}</textarea>
                        <div id="youtube-preview" class="d-flex flex-wrap gap-2 mt-2"></div>
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-bold small text-uppercase text-muted">Thư viện ảnh hiện tại</label>
                        @if(count($gallery) > 0)
                            <div class="d-flex flex-wrap gap-2 mb-2" id="existing-gallery">
                                @foreach($gallery as $img)
                                    <div class="position-relative mt-2 me-2 existing-image-container" style="width: 80px; height: 80px;">
                                        <input type="hidden" name="existing_images[]" value="{{ $img }}">
                                        <img src="{{ Storage::url($img) }}" alt="Image" class="rounded shadow-sm border border-secondary border-1 w-100 h-100" style="object-fit: cover;">
                                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger shadow-sm" style="cursor: pointer;" onclick="this.parentElement.remove()">
                                            <i class="fas fa-times"></i>
                                        </span>
                                    </div>
                                @endforeach
                        ...
                        @else
                            <div class="text-muted small mb-2">Chưa có ảnh thư viện.</div>
                        @endif
                        <label class="form-label fw-bold small text-uppercase text-muted mt-2">Thêm ảnh mới vào thư viện</label>
                        <input type="file" name="new_images[]" id="images-input" class="form-control" accept="image/*" multiple>
                        <div class="form-text small">Các ảnh cũ ở trên sẽ được giữ lại trừ khi bạn bấm xóa. Bạn có thể tải thêm nhiều ảnh mới ở đây.</div>
            ...
            <div class="tech-card mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="price" class="form-control" placeholder="0" min="0" value="{{ old('price', (int)$product->price) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="sale_price" class="form-control" placeholder="0" min="0" value="{{ old('sale_price', (int)$product->sale_price) }}">
                            <span class="input-group-text">₫</span>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-uppercase text-muted">Tồn kho</label>
                        <input type="number" name="stock" class="form-control" placeholder="0" min="0" value="{{ old('stock', $product->stock) }}">
                    </div>
                    <div class="mb-4">
...
<script>
    (function(){
        ...
        function renderYoutubePreview() {
            ...
            lines.forEach((line, index) => {
                ...
                if (match && match[1]) {
                    ...
                    removeBtn.innerHTML = '<i class="fas fa-times"></i>';
                    removeBtn.onclick = function() {
                        const newLines = videoInput.value.split('\n');
                        newLines.splice(index, 1);
                        videoInput.value = newLines.join('\n');
        ...
        const existingThumbContainer = document.getElementById('existing-thumbnail-container');
        ...
        if (thumbInput) {
            thumbInput.addEventListener('change', function() {
                ...
                const newPreviews = thumbPreview.querySelectorAll('.new-thumbnail-preview');
                newPreviews.forEach(el => el.remove());

                if (this.files && this.files[0]) {
                    // Hide existing image when new one is selected
                    if (existingThumbContainer) {
                        existingThumbContainer.style.display = 'none';
                        if (deleteThumbInput) deleteThumbInput.value = '1';
                    }
                    ...
                    if (file.type.startsWith('image/')) {
                        ...
                        wrapper.className = 'position-relative d-inline-block new-thumbnail-preview';
                        ...
                        removeBtn.onclick = function() {
                            thumbInput.value = '';
                            wrapper.remove();
                            if (fakeInput) fakeInput.value = '{{ $product->thumbnail ? basename($product->thumbnail) : 'Chưa có tệp nào được chọn' }}';
                            
                            // Restore existing image if we cancel new upload and didn't explicitly delete it before
                            if (existingThumbContainer && deleteThumbInput && deleteThumbInput.value === '1' && existingThumbContainer.style.display === 'none') {
                                existingThumbContainer.style.display = 'inline-block';
                                deleteThumbInput.value = '0';
                            }
                        };
                    ...
                    if (fakeInput) fakeInput.value = '{{ $product->thumbnail ? basename($product->thumbnail) : 'Chưa có tệp nào được chọn' }}';
                    if (existingThumbContainer && deleteThumbInput && deleteThumbInput.value === '1') {
                        existingThumbContainer.style.display = 'inline-block';
                        deleteThumbInput.value = '0';
                    }
                }
