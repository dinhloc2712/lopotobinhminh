...
<div class="tech-card mb-4">
    <div class="tech-header">
        <div><i class="fas fa-edit text-white-50 me-2"></i> Chỉnh sửa thông báo</div>
    </div>
    <div class="card-body p-4">
        <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <div class="mb-3">
                    <p class="mb-2 fw-medium">File hiện tại:</p>
                    <div class="d-flex flex-column gap-2">
                        @foreach($news->attachment as $index => $path)
                        <div class="d-flex align-items-center p-2 bg-light rounded border" style="max-width: 500px;">
                            <i class="fas fa-paperclip text-muted me-2"></i>
                            <span class="text-truncate flex-grow-1" title="{{ basename($path) }}">{{ basename($path) }}</span>
                            <div class="form-check mb-0 ms-3 text-nowrap">
                                <input class="form-check-input" type="checkbox" name="remove_attachments[{{ $index }}]" value="1" id="removeAttachment{{ $index }}">
                                <label class="form-check-label text-danger" for="removeAttachment{{ $index }}">Xóa file này</label>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <p class="mb-2 fw-medium">Thêm file mới:</p>
                ...
                <div class="form-text text-muted"><i class="fas fa-info-circle me-1"></i> Chọn thêm file mới để đính kèm. Dung lượng tối đa mỗi file 10MB.</div>
                @error('attachments')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                @error('attachments.*')
            ...
            <div class="card bg-light border-0 mb-4 p-4 rounded-4 shadow-sm">
                ...
                <div class="mb-3">
                    <div class="d-flex flex-wrap gap-4">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="recipient_type" id="recipientAll" value="all" {{ old('recipient_type', $news->recipient_type) == 'all' ? 'checked' : '' }} onchange="toggleRecipientOptions()">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="recipient_type" id="recipientRole" value="role" {{ old('recipient_type', $news->recipient_type) == 'role' ? 'checked' : '' }} onchange="toggleRecipientOptions()">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="recipient_type" id="recipientUser" value="user" {{ old('recipient_type', $news->recipient_type) == 'user' ? 'checked' : '' }} onchange="toggleRecipientOptions()">
                ...
                @php
                    $oldRoleIds = old('recipient_type') == 'role' ? old('recipient_ids', []) : ($news->recipient_type == 'role' ? $news->recipient_ids : []);
                    $oldUserIds = old('recipient_type') == 'user' ? old('recipient_ids', []) : ($news->recipient_type == 'user' ? $news->recipient_ids : []);
                @endphp

                <div id="roleSelectWrapper" class="mt-4" style="display: {{ old('recipient_type', $news->recipient_type) == 'role' ? 'block' : 'none' }};">
                    ...
                    <select class="form-select select2" name="recipient_ids[]" id="rolesSelect" multiple>
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" {{ in_array($role->id, $oldRoleIds ?? []) ? 'selected' : '' }}>
                ...
                <div id="userSelectWrapper" class="mt-4" style="display: {{ old('recipient_type', $news->recipient_type) == 'user' ? 'block' : 'none' }};">
                    ...
                    <select class="form-select select2" name="recipient_ids[]" id="usersSelect" multiple>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ in_array($user->id, $oldUserIds ?? []) ? 'selected' : '' }}>
