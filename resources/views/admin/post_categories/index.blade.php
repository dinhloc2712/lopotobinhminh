...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1 text-gray-800 fw-bold">Chuyên mục Landing Page</h1>
        <p class="mb-0 text-muted small">Quản lý các phân loại để tổ chức trang và bài viết</p>
    </div>
</div>
    ...
    <div class="tech-header" style="background: linear-gradient(135deg, #4e73df 0%, #224abe 100%); padding: 20px 25px;">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <h6 class="mb-0 fw-bold text-white d-flex align-items-center">
                <i class="fas fa-folder me-2 bg-white bg-opacity-25 rounded-circle p-2"
                   style="width: 36px; height: 36px; display: flex; align-items: center; justify-content: center;"></i>
                Danh sách Chuyên mục
            </h6>

            <form method="GET" action="{{ route('admin.post-categories.index') }}" class="d-flex align-items-center flex-wrap gap-2">
                {{-- Per Page --}}
            ...
            <table class="table table-modern mb-0">
                <thead>
                    <tr>
                        ...
                        <x-admin.table-header key="name" label="Tên chuyên mục" :sortColumn="$sortColumn" :sortOrder="$sortOrder" />
                        <x-admin.table-header key="slug" label="Đường dẫn (Slug)" :sortColumn="$sortColumn" :sortOrder="$sortOrder" />
                        <x-admin.table-header key="posts_count" label="Số bài viết" :sortColumn="$sortColumn" :sortOrder="$sortOrder" class="text-center" />
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3">{{ $category->posts_count }}</span>
                        </td>
                        <td>
                        ...
                        <td class="text-end pe-4">
                            ...
                            <form action="{{ route('admin.post-categories.destroy', $category->id) }}"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-primary text-white border-0">
                <h5 class="modal-title fw-bold" id="categoryModalLabel">Thêm chuyên mục mới</h5>
            ...
            <form id="categoryForm" action="{{ route('admin.post-categories.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-uppercase text-muted">Tên chuyên mục <span class="text-danger">*</span></label>
                        <input type="text" name="name" id="name" class="form-control rounded-3" placeholder="Ví dụ: Tin tức, Khuyến mãi..." required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-uppercase text-muted">Slug (Đường dẫn) <span class="text-danger">*</span></label>
                        <input type="text" name="slug" id="slug" class="form-control rounded-3" placeholder="ví-du-tin-tuc" required title="Chỉ dùng chữ cái không dấu, số và dấu gạch ngang">
                        <div class="form-text small">Tự động tạo từên chuyên mục hoặc nhập thủ công.</div>
                    </div>
                    <div class="mb-0">
                        <label class="form-label fw-bold small text-uppercase text-muted">Mô tả</label>
                        <textarea name="description" id="description" class="form-control rounded-3" rows="3" placeholder="Mô tả ngắn về chuyên mục..."></textarea>
                    </div>
                </div>
...
function resetModal() {
    document.getElementById('categoryModalLabel').innerText = 'Thêm chuyên mục mới';
...
function editCategory(category) {
    document.getElementById('categoryModalLabel').innerText = 'Chỉnh sửa chuyên mục';
    document.getElementById('categoryForm').action = `/admin/post-categories/${category.id}`;
...
function confirmDelete(id, name) {
    Swal.fire({
        ...
        html: `Xóa chuyên mục <strong>${name}</strong>? Hành động này không thể hoàn tác!`,
        icon: 'warning',
